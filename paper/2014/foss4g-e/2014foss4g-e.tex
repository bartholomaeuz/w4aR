...
By the way a OSRM Routing Service was chosen for the first test. On the app side a target position can be selected from a list and the actual position was used as start position. After the routing response is displayed in a new list (figure ~\ref{fig:app} on ~\pageref{fig:app}).
The user can explore the whole trip on the screen. Turn instruction by turn instruction.  
